...
class dBoxBasic(npyscreen.BoxBasic):
    def __init__(self, screen, footer=None, *args, **keywords):
        super(npyscreen.BoxBasic, self).__init__(screen, *args, **keywords)
    ...
    def update(self, clear=True):
        ...
        # draw corners
        self.parent.curses_pad.addch(self.relx, self.rely, curses.ACS_ULCORNER, )
        self.parent.curses_pad.addch(self.relx, self.rely+HEIGHT, curses.ACS_LLCORNER, )
        self.parent.curses_pad.addch(self.relx+WIDTH, self.rely, curses.ACS_URCORNER, )
        self.parent.curses_pad.addch(self.relx+WIDTH, self.rely+HEIGHT, curses.ACS_LRCORNER, )
...
class dBoxTitle(dBoxBasic):
    ...
    def update(self, clear=True):
        if self.hidden and clear:
            ...
            return False
        super(dBoxTitle, self).update(clear=clear)
        for w in self._my_widgets:
            w.update(clear=clear)
            
    def resize(self):
        super(dBoxTitle, self).resize()
